...
class NodeHandle(object):
    def __init__(self):
        ...
        self.ball_dis = rospy.Subscriber("/ball_dis",Int32,self.Set_ball_dis)
    ...
    def Set_ball_dis(self, msg):
        self.ball_dis = msg.data
        print("{}".format(self.ball_dis))
...
class Strategy(NodeHandle):
    ...
    def web_go_Strategy(self):  # chen
        if((len(self.web_axis) != 0 or len(self.web_ang) != 0) or self.state == 2):
            if(self.state == 0 or self.state == 2):
                ...
                if(abs(RPang) > self.error_ang):
                    z = self.z_max_speed if(RPang > 0) else -self.z_max_speed
                    self.Robot_Vel([0, z])
                else:
                    self.Robot_Stop()
                ...
                if(RPdis > self.error_dis):
                    if(self.prev_RPdis >= RPdis):
                        # print("{}  {}".format(self.prev_RPdis,RPdis))
                        x = self.x_speed_planning(RPdis)
                        z = self.z_speed_planning(RPang) if(
                            abs(RPang) > self.error_ang) else 0
                    else:
    ...
    def Catch_Ball_Strategy(self):  # chen
        ...
        if(self.state == 0):
            ...
            if(self._ballsColor == self.ballcolor):
                ...
                print("{}".format(color_to_strings(color)))
            if(color != None):
    ...
    def Short_Shoot_Strategy(self):  # chen
        ...
        if(self.state == 0):#set goal direction
            RPang = Norm_Angle(self.Get_RP_Angle(front_goal) - self._front)
            if(abs(RPang) > self.error_ang):
                z = self.find_ball_vel_z if(
                    RPang > 0) else -self.find_ball_vel_z
                ...
                self.Robot_Stop()
                self.state = 1
        elif(self.state == 1):#go to goal
            RPdis = self.Get_RP_Dis(front_goal)
            RPang = Norm_Angle(self.Get_RP_Angle(front_goal) - self._front)

            # print("goal = {} self = {} dis = {:.3f}  ang = {:.3f}".format(front_goal,self._pos,RPdis,RPang))#0000000
...
def main():
    ...
    #anonymous=true=>adds a random number to the end of your node's name, to make it unique.
    ...
    try:    #Blocks that monitor for errors
        rospy.spin()
    except KeyboardInterrupt:   #the errors you know will happend and what to do
        print("Shutting down")
